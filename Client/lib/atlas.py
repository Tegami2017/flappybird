...
atlas['hero']={"width":48, "height":48, "x":56, "y":970}
...
class CollidableAnimatingSprite(cocos.sprite.Sprite):       # 创建一只不会扇动翅膀的小鸟sprite
...
class CollidableAnimatingSprite0(cocos.sprite.Sprite):      # 创建另一只一直扇动翅膀的小鸟sprite
    def __init__(self, imagename, center_x, center_y, radius):
        actionimage_list = []  # 动画图片列表
        for i in range(3):  # 生成三个动画图片对象 ，放进actionimage_list
            actionimage_list.append(image.AnimationFrame(common.load_image(imagename + str(i) + ".png"), 0.1))
        actionimage = image.Animation(actionimage_list)
        super(CollidableAnimatingSprite0, self).__init__(actionimage)       # 生成sprite
        self.cshape = CircleShape(Vector2(center_x, center_y), radius)
        self.position = (center_x, center_y)
        self.name = image
        self.gravity = 0
