...
def addCollision(gameScene, gameLayer, spriteBird, pipes, land_1, land_2):
    ...
    from pipe import current_pipeDistance
    # 设置land区域对应的刚体
    landSprite = CollidableRectSprite("land", (common.visibleSize["width"]) / 2, (atlas["land"]["height"] / 4 - 3),
                                      (common.visibleSize["width"]) / 2, (atlas["land"]["height"]) / 2)
    ...
    def collisionHandler(dt):
        ...
        for i in range(0, pipeCount):
            pipes[i].get("downPipe").cshape.center = Vector2(pipes[i].position[0], pipes[i].position[1] + (
            atlas["pipe_up"]["height"] + current_pipeDistance))
        ...
        for other in collision_manager.iter_colliding(spriteBird):
            if other.name == 'land':
                ...
                for i in range(0, pipeCount):
                    if (pipes[i].position[0] - atlas["pipe_up"]["width"] / 2) <= birdXPosition and (
                        pipes[i].position[0] + atlas["pipe_up"]["width"] / 2) >= birdXPosition:
