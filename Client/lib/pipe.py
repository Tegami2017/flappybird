...
pipeHeight = 320
pipeWidth = 52
current_pipeDistance = None    #上下管道间的距离
# pipeInterval = 180    #两根管道的水平距离
waitDistance = 100    #开始时第一根管道距离屏幕最右侧的距离
# heightOffset = 25     #管道的高度偏移值

game_level_dic = {1: {'pipeDistance': 130, 'pipeInterval': 250, 'heightOffset': (25, 25), 'speed': 30},
                  2: {'pipeDistance': 100, 'pipeInterval': 180, 'heightOffset': (20, 30), 'speed': 130},
                  3: {'pipeDistance': 100, 'pipeInterval': 180, 'heightOffset': (15, 150), 'speed': 150},
                  4: {'pipeDistance': 80, 'pipeInterval': 100, 'heightOffset': (3, 180), 'speed': 180},
                  }
...
def createPipes(layer, gameScene, spriteBird, score, level_num):
    global g_score, movePipeFunc, calScoreFunc, game_level_dic, current_pipeDistance
    level = game_level_dic[level_num]
    pipeDistance, pipeInterval, heightOffset_tup, move_speed = \
        level['pipeDistance'], level['pipeInterval'], level['heightOffset'], level['speed']
    current_pipeDistance = pipeDistance
    def initPipe():
        for i in range(0, pipeCount):
            heightOffset = random.randint(heightOffset_tup[0], heightOffset_tup[1])
    ...
    def movePipe(dt):
        moveDistance = common.visibleSize["width"]/(move_speed)   # 移动速度和land一致
        for i in range(0, pipeCount):
